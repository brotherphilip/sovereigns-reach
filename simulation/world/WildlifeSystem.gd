extends RefCounted
# Wandering animal herds (deer to start). Pure-data simulation: every animal is a
# JSON-serializable Dictionary in GameState.wildlife, advanced deterministically
# from a seeded RNG each tick. Herding (boids-lite cohesion + separation),
# slow roaming, a state machine (roam / feed / brood / run), threat flight, and
# slow breeding. The view layer reads these fields to render & animate.
...
const HERD_CAP: int = 9            # max animals per herd
...
static func make_animal(id: int, herd_id: int, x: float, y: float, adult: bool) -> Dictionary:
	return {
		"id": id, "type": "deer", "herd_id": herd_id,
		"x": x, "y": y, "vx": 0.0, "vy": 0.0,
		"state": STATE_ROAM, "state_ticks": 1,
		"facing": 1.0, "age": ADULT_AGE if adult else 0,
		"hp": ADULT_MAX_HP if adult else FAWN_MAX_HP,
		"max_hp": ADULT_MAX_HP if adult else FAWN_MAX_HP,
		"is_alive": true, "anim": 0.0,
	}

# Spawns a herd of `count` deer clustered near (cx,cy). next_id is the first id to
# use; returns the next free id after the herd.
static func spawn_herd(wildlife: Array, herd_id: int, cx: float, cy: float, count: int,
	...
	for _i in range(count):
		...
		var adult: bool = _i < count - 1   # one fawn per herd to start
		wildlife.append(make_animal(next_id, herd_id, cx + ox, cy + oy, adult))
		next_id += 1
	return next_id
...
static func _tick_animal(a: Dictionary, centroid: Vector2, wildlife: Array,
	...
	if threat != Vector2.INF:
		...
		a["vx"] = away.x * RUN_SPEED
		a["vy"] = away.y * RUN_SPEED
	else:
		...
		match a.get("state", STATE_ROAM):
			...
			STATE_ROAM:
				...
				a["vx"] = steer.x * ROAM_SPEED
				a["vy"] = steer.y * ROAM_SPEED
				if a["state_ticks"] <= 0:
					_enter(a, _pick_idle(rng), rng)
...
static func _pick_idle(rng: RandomNumberGenerator) -> String:
...
static func _maybe_breed(wildlife: Array, sums: Dictionary, rng: RandomNumberGenerator,
	...
	for hid in sums.keys():
		...
		if herd_size >= HERD_CAP or adults_by_herd.get(hid, 0) < 2:
			continue
