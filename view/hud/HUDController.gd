...
static func get_popularity_breakdown_tooltip(player: Dictionary) -> String:
	...
	var religion:    float = player.get("religion_coverage", 0.0)
	...
	var total: float = float(food_delta + variety_bonus + ale_delta + int(religion) + tax_delta)
	var lines: Array = [
...
static func get_weather_tooltip(weather: Dictionary) -> String:
	...
	var lines: Array = ["Weather: %s" % weather.get("current_name", "Unknown")]
	if pop != 0.0:
